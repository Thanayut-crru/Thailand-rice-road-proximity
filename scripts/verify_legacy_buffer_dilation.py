#!/usr/bin/env python
"""Verify whether an archived two-pixel project buffer is dilation of its one-pixel buffer.
...
def main():
    ...
    with rasterio.open(one_path) as one, rasterio.open(two_path) as two:
        if one.shape != two.shape or one.transform != two.transform or one.crs != two.crs:
            raise ValueError("The two archived project rasters do not share one grid")
        ...
        result = {
            ...
            "interpretation": (
                "The archived project B2 raster is exactly the one-pixel binary dilation "
                "of archived B1 on the shared EPSG:4326 grid. Its archived 50 m label "
